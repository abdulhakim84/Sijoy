...
// Menyertakan file wajah dari folder face
    ...
    board.SetNetworkEventCallback([this](NetworkEvent event, const std::string& data) {
        ...
        switch (event) {
            case NetworkEvent::Scanning:
               xEventGroupSetBits(event_group_, MAIN_EVENT_NETWORK_DISCONNECTED);
                if (display) {
                    ...
                    display->SetIcon("wifi");
                }
                break;
            case NetworkEvent::Connecting: {
                if (data.empty()) {
                    if (display) {
                        display->SetStatus(Lang::Strings::CONNECTING_CELLULAR);
                        display->SetIcon("square_rss");
                    }
                } else {
            ...
            case NetworkEvent::Disconnected:
                xEventGroupSetBits(event_group_, MAIN_EVENT_NETWORK_DISCONNECTED);
                if (display) {
                    display->SetStatus(Lang::Strings::DISCONNECTED);
                    display->SetIcon("wifi_slash");
                }
                break;
            ...
            case NetworkEvent::ModemDetecting:
                if (display) {
                    display->SetStatus(Lang::Strings::MODEM_DETECTING);
                    display->SetIcon("square_rss");
                }
                break;
...
void Application::Run() {
    ...
    while (true) {
        ...
        // Eksekusi animasi render wajah dan fisika setiap siklus loop
        updateFacePhysics();
        updateLook();
        updateBlink();
        renderFaceLoop();
        ...
        if (strcmp(type->valuestring, "tts") == 0) {
            ...
            if (strcmp(state->valuestring, "start") == 0) {
                ...
                if (cJSON_IsString(text)) {
                    ...
                    auto display = Board::GetInstance().GetDisplay();
                    if (display) {
                        display->SetSubtitle(text->valuestring);
                    }
                }
            }
            ...
            if (cJSON_IsString(text)) {
                ...
                auto display = Board::GetInstance().GetDisplay();
                if (display) {
                    display->SetStatus(text->valuestring);
                }
            }
        } else if (strcmp(type->valuestring, "llm") == 0) {
            // Membaca respon emosi format JSON dari server LLM, lalu mengubah variabel global wajah
            ...
            if (cJSON_IsObject(payload)) {
                Schedule([this, payload_str = std::string(cJSON_PrintUnformatted(payload))]() {
                    auto display = Board::GetInstance().GetDisplay();
                    if (display) {
                        display->ReceiveCustomMessage(payload_str);
                    }
                });
            }
#endif
...
void Application::Alert(const char* status, const char* message, const char* emotion,
    ...
    if (display) {
        ...
        display->SetSubtitle(message);
        display->SetIcon(emotion);
    }
    if (!sound.empty()) {
