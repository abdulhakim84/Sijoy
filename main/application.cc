...
// Menyertakan file wajah dari folder face untuk mengendalikan animasi OLED secara mandiri
...
void Application::Initialize() {
    ...
    // Inisialisasi layar OLED melalui fungsi buatan di wajah.cpp
    initFaceDisplay();
    ...
    // Set network event callback for network state handling tanpa menyentuh class Display bawaan
    board.SetNetworkEventCallback([this](NetworkEvent event, const std::string& data) {
        switch (event) {
            case NetworkEvent::Scanning:
                xEventGroupSetBits(event_group_, MAIN_EVENT_NETWORK_DISCONNECTED);
                ESP_LOGI(TAG, "Network Scanning WiFi...");
                break;
            case NetworkEvent::Connecting:
                ESP_LOGI(TAG, "Network Connecting...");
                break;
            case NetworkEvent::Connected:
                ESP_LOGI(TAG, "Network Connected to %s", data.c_str());
            ...
            case NetworkEvent::WifiConfigModeEnter:
                ESP_LOGI(TAG, "Network WiFi Config Mode Enter");
                break;
...
void Application::Run() {
    ...
    while (true) {
        ...
        // Eksekusi animasi render wajah dan fisika setiap siklus loop pada layar OLED
        updateFacePhysics();
        updateLook();
        updateBlink();
        renderFaceLoop();
...
void Application::CheckAssetsVersion() {
    ...
    if (!download_url.empty()) {
        ...
        bool success =
            assets.Download(download_url, [this](int progress, size_t speed) -> void {
                ESP_LOGI(TAG, "Downloading assets: %d%%, Speed: %zu KB/s", progress, speed / 1024);
            });
...
void Application::InitializeProtocol() {
    ...
    protocol_->OnIncomingJson([this](const cJSON* root) {
        ...
        if (strcmp(type->valuestring, "tts") == 0) {
            ...
            // Membaca respon emosi format JSON dari server LLM, lalu mengubah variabel emosi wajah global
...
void Application::DismissAlert() {
    // Kosong karena render beralih ke engine animasi wajah di wajah.cpp
}
